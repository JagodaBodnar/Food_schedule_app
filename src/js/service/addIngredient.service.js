...
export const inputTouched = (value) => {
...
export const filterIngredients = (event) => {
...
export const checkIfStillTyping = (event) => {
    const checking = setInterval(() => {
        clearInterval(checking)
        filterIngredients(event)
    }, 1000)
}
export const filterIngredientsByCategory = (event) => {
...
export const addFilterBackgroundColor =(event)=>{
    const selectAllCategories =  document.querySelectorAll('.category_tile')
    selectAllCategories.forEach(item=>item.style.backgroundColor = 'transparent')
